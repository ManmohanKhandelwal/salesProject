export const SQLSelect = ({
  ...
  try {
    if (!Array.isArray(Queries) || Queries.length === 0)
      throw { message: "Queries must be a non-empty array", status: 400 };

    if (typeof TableName !== "string" || TableName.trim() === "")
      throw { message: "TableName must be a non-empty string", status: 400 };
    ...
    console.error("Error at SQLSelect:", error?.message || error);
  }
};
