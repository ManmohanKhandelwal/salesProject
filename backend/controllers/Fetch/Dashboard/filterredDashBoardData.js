...
export const getFilteredDashBoardData = async (req, res) => {
  try {
    ...
    const filteredQueries = Object.entries(queries).reduce((acc, [key, value]) => {
      if (value.length > 0 && !value.includes("all")) {
        acc[key] = key === "months" ? value.map(month => monthMapping[month] || month) : value;
      }
      return acc;
    }, {});
    ...
    if (requiresStoreMapping) {
      fromClause += " LEFT JOIN store_mapping ON pd.customer_code = store_mapping.Old_Store_Code";
    }
    if (requiresChannelMapping) {
      fromClause += " LEFT JOIN channel_mapping ON pd.channel_description = channel_mapping.channel";
    }
  ...
  const sqlQueryRetailingByChannel = `
    SELECT cm.broad_channel, SUM(pd.retailing) AS totalRetailing
    ${fromClause}
  ...
  const sqlQueryRetailingByCategory = `
  ...
  const sqlQueryRetailTrend = `
    SELECT YEAR(pd.document_date) AS year, MONTH(pd.document_date) AS month, SUM(pd.retailing) AS totalRetailing
    ${fromClause}
    ${whereClause}
  ...
  `;
    ...
    const [
      ...
      connection.query(sqlQueryRetailingByChannel, queryParams),
      connection.query(sqlQueryRetailingByCategory, queryParams),
      connection.query(sqlQueryRetailTrend, queryParams),
